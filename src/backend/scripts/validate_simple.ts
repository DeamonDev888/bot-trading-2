...
class SimpleDataValidator {
  ...
  async generateReport(): Promise<SimpleValidationReport> {
    console.log('🔍 Génération du rapport de validation...');

    const report: SimpleValidationReport = {
      totalNews: 0,
      recentNews24h: 0,
      recentNews7d: 0,
      duplicates: 0,
  ...
  private detectIssues(report: SimpleValidationReport): void {
  ...
  private calculateQualityScore(report: SimpleValidationReport): void {
    ...
    if (report.recentNews24h > 100) score += 10;
    else if (report.recentNews24h > 50) score += 5;

    if (report.recentNews7d > 500) score += 5;
    else if (report.recentNews7d > 100) score += 2;
  ...
  private generateRecommendations(report: SimpleValidationReport): void {
    console.log('💡 Génération des recommandations...');

    if (report.duplicates > 0) {
      report.recommendations.push(
        '💡 Implémenter une déduplication robuste basée sur le hash du contenu'
      );
      report.recommendations.push(
        '💡 Ajouter des contraintes UNIQUE au niveau de la base de données'
      );
    }
    ...
    if (sourceCount < 3) {
      report.recommendations.push(
        '💡 Diversifier les sources de données pour réduire la dépendance'
      );
    }
  ...
  async printReport(report: SimpleValidationReport): Promise<string> {
    const lines: string[] = [];
