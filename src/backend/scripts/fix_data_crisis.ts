...
class DataCrisisFixer {
      ...
      const emergencyNews = [
        {
          ...
          content:
            'Les marchés asiatiques ont ouvert en hausse, avec le Nikkei gagnant 1.2% suite à des annonces positives sur le commerce. Les investisseurs restent cependant prudents face aux perspectives économiques mondiales incertaines.',
        ...
        },
        {
          ...
          content:
            "La Réserve Fédérale américaine a décidé de maintenir les taux d'intérêt inchangés, citant une inflation qui montre des signes de modération malgré des tensions persistantes sur les prix.",
        ...
        },
        {
          ...
          content:
            "Les cours du pétrole ont rebondi de 3% après que l'OPEC+ a annoncé des réductions de production pour stabiliser les prix. Cette décision soutient les marchés énergétiques mondiaux.",
        ...
        },
        {
          ...
          content:
            "Les valeurs technologiques européennes subissent une pression à la vente face aux craintes de régulation et à un environnement de taux d'intérêt plus élevé. L'indice STOXX Tech perd 2.1%.",
        ...
        },
        {
          ...
          content:
            "L'euro s'échange en baisse face au dollar américain, les investisseurs privilégiant les actifs refuges amid les tensions géopolitiques croissantes et l'incertitude économique.",
        ...
        },
        {
          ...
          content:
            "Le Bitcoin dépasse le seuil psychologique de 45000$ amid un regain d'intérêt des investisseurs institutionnels. L'ensemble du marché des cryptomonnaies suit la tendance avec des gains généralisés.",
        ...
        },
        {
          ...
          content:
            'Le gouvernement chinois a dévoilé un ensemble de mesures pour soutenir la croissance économique, incluant des réductions fiscales et des investissements dans les infrastructures. Les marchés réagissent positivement.',
        ...
        },
        {
          ...
          content:
            "Les derniers chiffres sur l'emploi aux États-Unis montrent une création d'emplois inférieure aux attentes, soulevant des questions sur la résilience du marché du travail américain.",
        ...
        },
        {
          ...
          content:
            'Le secteur immobilier américain montre des signes de stabilisation avec des prix qui se maintiennent et des volumes de ventes en légère augmentation. Les taux hypothécaires restent cependant un facteur de préoccupation.',
        ...
        },
        {
          ...
          content:
            'Actions Tesla en baisse de 4% après que des concurrents traditionnels ont annoncé de nouvelles stratégies électriques. Le secteur automobile fait face à une intensification de la concurrence.',
        ...
        },
        {
          ...
          content:
            'Les principaux indices européens (CAC 40, DAX, FTSE) ouvrent en hausse, tirés par la performance positive des marchés asiatiques et des attentes de politiques monétaires accommodantes.',
        ...
        },
        {
          ...
          content:
            "Le prix de l'or atteint son plus haut niveau depuis six mois, les investisseurs cherchant refuge face aux incertitudes économiques et géopolitiques croissantes.",
        ...
        },
        {
          ...
          content:
            "L'inflation dans la zone euro persiste au-dessus de la cible de 2% de la Banque Centrale Européenne, compliquant les décisions politiques monétaires futures.",
        ...
        },
        {
          ...
          content:
            'La demande de puces pour intelligence artificielle explose, avec des fabricants comme NVIDIA et TSMC faisant face à des carnets de commandes records. Le secteur devrait continuer sa forte croissance.',
        ...
        },
        {
          ...
          content:
            "Le marché obligataire mondial subit une pression avec des rendements qui augmentent, les investisseurs s'attendant à des politiques monétaires moins accommodantes dans les mois à venir.",
  ...
  formatReport(result: DataCrisisFixResult): string {
    ...
    lines.push(
      `   • Total news      ${result.initialStats.totalNews.toString().padStart(8)}        ${result.finalStats.totalNews.toString().padStart(8)}        ${result.finalStats.totalNews - result.initialStats.totalNews > 0 ? '+' : ''}${(result.finalStats.totalNews - result.initialStats.totalNews).toString().padStart(8)}`
    );
    lines.push(
      `   • News 24h        ${result.initialStats.recentNews24h.toString().padStart(8)}        ${result.finalStats.recentNews24h.toString().padStart(8)}        ${result.finalStats.recentNews24h - result.initialStats.recentNews24h > 0 ? '+' : ''}${(result.finalStats.recentNews24h - result.initialStats.recentNews24h).toString().padStart(8)}`
    );
    lines.push(
      `   • News 48h        ${result.initialStats.recentNews48h.toString().padStart(8)}        ${result.finalStats.recentNews48h.toString().padStart(8)}        ${result.finalStats.recentNews48h - result.initialStats.recentNews48h > 0 ? '+' : ''}${(result.finalStats.recentNews48h - result.initialStats.recentNews48h).toString().padStart(8)}`
    );
    lines.push('');
    ...
    lines.push(
      `   • News 24h: ${result.finalStats.recentNews24h >= target24h ? '✅ Atteint' : '❌ Manqué'} (${result.finalStats.recentNews24h}/${target24h})`
    );
    lines.push(
      `   • News 48h: ${result.finalStats.recentNews48h >= target48h ? '✅ Atteint' : '❌ Manqué'} (${result.finalStats.recentNews48h}/${target48h})`
    );
    lines.push('');
