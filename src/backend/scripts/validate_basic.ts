...
class BasicValidator {
  ...
  async generateReport(): Promise<BasicValidationReport> {
    const report: BasicValidationReport = {
      connected: false,
      totalNews: 0,
      recentNews24h: 0,
      recentNews7d: 0,
  ...
  private detectIssues(report: BasicValidationReport): void {
  ...
  async printReport(report: BasicValidationReport): Promise<string> {
    const lines: string[] = [];
    ...
    lines.push(
      `${statusEmoji} Connexion à la base de données: ${report.connected ? 'Établie' : 'Échouée'}`
    );
    lines.push('');
