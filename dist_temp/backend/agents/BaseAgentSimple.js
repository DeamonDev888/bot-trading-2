...
class BaseAgentSimple {
    constructor(name) {
        this.agentName = name;
        this.dataDir = path_1.default.join(process.cwd(), 'data', 'agent-data', name);
    }
    /**
     * Exécute KiloCode avec une approche robuste et simple
     */
    async callKiloCode(req) {
        const fullOutputPath = path_1.default.join(process.cwd(), req.outputFile);
    ...
    async executeWithFile(req, fullOutputPath) {
        const tempPromptPath = path_1.default.join(process.cwd(), 'temp_prompt.txt');
        await promises_1.default.writeFile(tempPromptPath, req.prompt, 'utf-8');
        ...
        try {
            ...
            await promises_1.default.writeFile(fullOutputPath, stdout, 'utf-8');
            return this.parseKiloCodeOutput(stdout);
        }
        finally {
            // Nettoyer le fichier temporaire
            try {
                await promises_1.default.unlink(tempPromptPath);
    ...
    async executeDirect(req, fullOutputPath) {
        ...
        if (req.inputFile) {
            const fullInputPath = path_1.default.join(process.cwd(), req.inputFile);
