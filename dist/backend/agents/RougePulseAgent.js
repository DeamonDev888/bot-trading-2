...
export class RougePulseAgent extends BaseAgentSimple {
     ...
     */
    async filterCalendarEvents(startDate, endDate) {
     ...
     */
    async applyExpertFiltering(events) {
     ...
     */
    classifyEventsWithScoring(events) {
        const now = new Date();
        ...
        Object.keys(classified).forEach(key => {
            classified[key].sort((a, b) => {
                const scoreA = a.calculated_score || a.importance || 0;
                const scoreB = b.calculated_score || b.importance || 0;
                if (scoreB !== scoreA) {
     ...
     */
    getMarketSensitivity(eventName) {
     ...
     */
    generateAnalysisSummary(classified, marketMovers) {
     ...
     */
    calculateAdvancedVolatilityScore(classified) {
        let score = 0;
        ...
        const now = new Date();
        const next24h = new Date(now.getTime() + 24 * 60 * 60 * 1000);
        [...classified.critical, ...classified.high, ...classified.medium].forEach((event) => {
            const eventDate = new Date(event.event_date);
    ...
    getNext24HoursAlerts(classified) {
        ...
        return [...classified.critical, ...classified.high, ...classified.medium]
            .filter((event) => new Date(event.event_date) <= next24h)
     ...
     */
    calculateSurprisePotential(event) {
        const importance = event.importance || 1;
        const isKeyIndicator = this.isKeyMarketIndicator(event.event_name);
        if (importance >= 3 || (importance >= 2 && isKeyIndicator))
            return 'HIGH';
        if (importance >= 2 || isKeyIndicator)
            return 'MEDIUM';
        return 'LOW';
    }
    /**
     ...
     */
    calculateFilterConfidence(classified) {
        const totalEvents = Object.values(classified).reduce((sum, events) => sum + events.length, 0);
        if (totalEvents === 0)
            return 0;
        ...
        const hasKeyIndicators = [...classified.critical, ...classified.high, ...classified.medium]
            .some((event) => this.isKeyMarketIndicator(event.event_name));
        if (hasKeyIndicators)
            confidence += 0.15;
        // +10% si les données sont récentes
        const hasRecentData = [...classified.critical, ...classified.high]
            .some((event) => this.hasRecentData(event));
        if (hasRecentData)
            confidence += 0.1;
