            ...
            for (const row of urlsResult.rows) {
                try {
                    ...
                    if (normalizedUrl !== row.url) {
                        await client.query('UPDATE news_items SET url = $1 WHERE id = $2', [normalizedUrl, row.id]);
                        normalized++;
                    }
                }
                catch (e) {
      ...
      `);
            ...
            for (const row of itemsResult.rows) {
                ...
                const urlHash = crypto.createHash('sha256')
                    ...
                    .digest('hex');
                await client.query('UPDATE news_items SET title_hash = $1, url_hash = $2 WHERE id = $3', [titleHash, urlHash, row.id]);
                updated++;
            }
...
async function main() {
    ...
    const report = { timestamp: new Date().toISOString(), mode: analyzeOnly ? 'analyze' : 'cleanup' };
    try {
